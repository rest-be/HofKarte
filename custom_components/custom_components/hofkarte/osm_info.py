...
async def _rufe_overpass_ab(session: aiohttp.ClientSession, query: str) -> dict[str, Any]:
    ...
    Jeder Fehlerfall wird zusätzlich über ``_LOGGER`` protokolliert (Level
    ``warning``, sichtbar in den Home-Assistant-Protokollen auch ohne
    aktiviertes Debug-Logging): Die an die Verwaltungsoberfläche
    zurückgegebene Fehlermeldung ist bewusst allgemein gehalten (siehe
    ``management.ws_osm_info``), enthält also z. B. nicht den tatsächlichen
    HTTP-Status, den Antworttext der Overpass API oder die genaue
    Verbindungsfehler-Ursache (DNS, TLS, Timeout, Firewall) - all das lässt
    sich bei Bedarf nur über die Protokollierung hier nachvollziehen."""
    ...
    try:
        async with session.post(
            ...
            if antwort.status >= 400:
                ...
                _LOGGER.warning(
                    "Overpass API hat Fehlerstatus %s zurückgegeben: %s",
                    antwort.status,
                    fehlertext,
                )
            ...
            async for chunk in antwort.content.iter_chunked(_LESE_CHUNK_BYTES):
                ...
                if len(rohdaten) > MAX_ANTWORT_BYTES:
                    _LOGGER.warning(
                        "Antwort der Overpass API überschreitet das "
                        "Grössenlimit von %s Bytes.",
                        MAX_ANTWORT_BYTES,
                    )
                    raise OsmNichtErreichbarError(
            ...
            try:
                ...
                _LOGGER.warning(
                    "Antwort der Overpass API konnte nicht als JSON gelesen "
                    "werden: %s",
                    err,
                )
                raise OsmNichtErreichbarError(
        ...
        _LOGGER.warning(
            "Overpass API (%s) nicht erreichbar: %s: %s",
            OVERPASS_URL,
            type(err).__name__,
            err,
        )
