class HofkartePanel extends HTMLElement {
  ...
  empty() {
    return { id: "", name: "", beschreibung: "", adresse: "", plz: "", ort: "", land: "", latitude: "", longitude: "", oeffnungszeiten: [], sonderoeffnungszeiten: [], produkte: [], kategorien: [], zahlungsarten: [], verkaufsarten: [], merkmale: [] };
  }
  ...
  validate(d) {
    ...
    for (const row of d.oeffnungszeiten) {
      if (!row.beginn || !row.ende || !timeRe.test(row.beginn) || !timeRe.test(row.ende) || row.beginn === row.ende) throw new Error("Öffnungszeiten enthalten ungültige Zeiten. Bitte HH:MM verwenden.");
    }
    for (const row of d.sonderoeffnungszeiten) {
      if (row.geschlossen) continue;
      if ((row.beginn && !timeRe.test(row.beginn)) || (row.ende && !timeRe.test(row.ende))) throw new Error("Sonderöffnungszeiten enthalten ungültige Zeiten. Bitte HH:MM verwenden.");
    }
  }
  ...
  editor() {
    ...
    const specials=(d.sonderoeffnungszeiten||[]).map(x=>`<div class="special" data-special><label>Von<input type=date name=datum_von value="${x.datum_von||""}"></label><label>Bis<input type=date name=datum_bis value="${x.datum_bis||""}"></label><label>Beginn<input type="text" inputmode="numeric" autocomplete="off" maxlength="5" pattern="(?:[01]\d|2[0-3]):[0-5]\d" placeholder="HH:MM" name="beginn" value="${x.beginn||""}"></label><label>Ende<input type="text" inputmode="numeric" autocomplete="off" maxlength="5" pattern="(?:[01]\d|2[0-3]):[0-5]\d" placeholder="HH:MM" name="ende" value="${x.ende||""}"></label><label>Geschlossen<input type=checkbox name=geschlossen ${x.geschlossen?"checked":""}></label><button type=button class=secondary data-remove-special>−</button></div>`).join("");
  ...
  opening(x){return `<div class=opening data-opening data-day="${x.day}"><div class=day>${x.label}</div><label>Von<input type="text" inputmode="numeric" autocomplete="off" maxlength="5" pattern="(?:[01]\d|2[0-3]):[0-5]\d" placeholder="HH:MM" name="beginn" value="${x.beginn||""}"></label><label>Bis<input type="text" inputmode="numeric" autocomplete="off" maxlength="5" pattern="(?:[01]\d|2[0-3]):[0-5]\d" placeholder="HH:MM" name="ende" value="${x.ende||""}"></label><button type=button class=secondary data-remove-opening>−</button></div>`}
  bind(){
    ...
    this.shadowRoot.querySelector("[data-add-special]")?.addEventListener("click",()=>{const el=document.createElement("div");el.innerHTML=`<div class="special" data-special><label>Von<input type=date name=datum_von></label><label>Bis<input type=date name=datum_bis></label><label>Beginn<input type="text" inputmode="numeric" autocomplete="off" maxlength="5" pattern="(?:[01]\d|2[0-3]):[0-5]\d" placeholder="HH:MM" name="beginn"></label><label>Ende<input type="text" inputmode="numeric" autocomplete="off" maxlength="5" pattern="(?:[01]\d|2[0-3]):[0-5]\d" placeholder="HH:MM" name="ende"></label><label>Geschlossen<input type=checkbox name=geschlossen></label><button type=button class=secondary data-remove-special>−</button></div>`;this.shadowRoot.querySelector("#specials").append(el.firstElementChild);this.bind()});
  }
